...
export default async function SearchFilters({ type, genre, sort, query, showAllGenres = false }: FiltersProps) {
  ...
  const visibleGenresMobile = showAllGenres ? allGenres : allGenres.slice(0, maxGenresMobile)
  const visibleGenresDesktop = showAllGenres ? allGenres : allGenres.slice(0, maxGenresDesktop)
    ...
    <div className='space-y-6'>
      {!query && (
        <div className='flex flex-wrap items-center gap-2'>
          {(['all', 'movie', 'tv'] as const).map(t => (
            ...
            >
              <Link href={buildHref({ type: t === 'all' ? undefined : t, genre, sort, showAllGenres: showAllGenres ? '1' : undefined })}>
      ...
      <div className='space-y-2'>
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center gap-3'>
            ...
            {showAllGenres && (
              <Link
                href={buildHref({ type, genre, sort, showAllGenres: undefined })}
                className='text-xs text-muted-foreground hover:text-foreground'
              >
                Show less
              </Link>
            ...
            {genre && (
              <Link
                href={buildHref({ type, sort, showAllGenres: showAllGenres ? '1' : undefined })}
                className='text-xs text-muted-foreground hover:text-foreground'
              >
                Clear
              </Link>
          ...
          {visibleGenresMobile.map(g => {
            ...
            return (
              <Badge
                ...
                className='cursor-pointer'
              >
                <Link href={buildHref({ type, genre: String(g.id), sort, showAllGenres: showAllGenres ? '1' : undefined })}>
                  {g.name}
                </Link>
              </Badge>
            )
      ...
      <div className='space-y-2'>
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <button type='button' className='text-muted-foreground hover:text-foreground' aria-label='Sort information'>
                  <Info className='h-3.5 w-3.5' />
                </button>
              </TooltipTrigger>
              <TooltipContent side='top' className='max-w-[232px] text-xs '>
                Sorting by Rating or Release Date may show less relevant results. Use Popularity for the best experience.
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        <div className='flex flex-wrap gap-2'>
          {sorts.map(s => {
            const selected = sort === s.id || (!sort && s.id === 'popularity.desc')
            return (
              <Badge
                key={s.id}
                asChild
                ...
                className='cursor-pointer'
              >
                <Link href={buildHref({ type, genre, sort: s.id, showAllGenres: showAllGenres ? '1' : undefined })}>{s.label}</Link>
              </Badge>
            )
          })}
        </div>
